...
def update_model():
    ...
    #use current model to evaluate action action argmax_a' Q_current(s', a')_
    next_action_batch = model(next_state_batch)
    #next_action_batch = torch.cat(tuple(torch.max(next_action_batch[i]) for i in range(len(batch))))
    # use target network to evaluate value
    # y = r + discount_factor * Q_tar(s', a')
    next_state_batch = target_model(next_state_batch)
    max = next_action_batch[0]
    max_num = 0
    for i in range(0,len(batch)):
        if next_action_batch[i].max() == next_action_batch.max():
            print("hh")
            max = next_action_batch[i]
            max_num = i

    #y_batch = (reward_batch[i] if batch[i][4] else reward_batch[i] for i in range(len(batch))) + torch.mul(DISCOUNT_FACTOR, torch.sum(target_model(next_state_batch) * torch.max(next_action_batch[i], dim=1) for i in range(len(batch))))

    #y_batch = torch.cat(tuple(reward_batch[i] if batch[i][4] else reward_batch[i] + DISCOUNT_FACTOR * torch.sum(next_state_batch * action_batch, dim=1) for i in range(len(batch))))
    #y_batch = torch.cat(tuple(reward_batch[i] if batch[i][4] else reward_batch[i] + DISCOUNT_FACTOR * torch.max(next_action_batch[i]) for i in range(len(batch))))

    y_batch = torch.stack(tuple(reward_batch[max_num] if batch[max_num][4] else reward_batch[max_num] + DISCOUNT_FACTOR * torch.sum(next_state_batch * max, dim=1)))
